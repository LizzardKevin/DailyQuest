    ...
    var body: some View {
        NavigationStack {
            ZStack {
                ...
                ScrollView {
                    VStack(spacing: 18) {
                        ScreenHeader("设置", subtitle: "外观、提醒、任务与教程")
                            ...
                            VStack(alignment: .leading, spacing: 10) {
                                ...
                                if !APIConfig.isConfigured {
                                    ...
                                    Text(APIConfig.baseURLString)
                                        ...
                                        .foregroundStyle(AppTheme.inkMuted)
                                        .lineLimit(2)
                                    Text("拆解：\(APIConfig.breakdownURL.absoluteString)")
                                        .font(.system(size: 10, design: .monospaced))
                                        .foregroundStyle(AppTheme.inkMuted)
                                        .lineLimit(2)
                                    Text("奖牌：\(APIConfig.medalDesignURL.absoluteString)")
                                        .font(.system(size: 10, design: .monospaced))
                                        .foregroundStyle(AppTheme.inkMuted)
                                        .lineLimit(2)
                                }
                        ...
                        GlassCard {
                            VStack(alignment: .leading, spacing: 8) {
                                Text("隐私")
                                    .font(AppTheme.caption())
                                    .foregroundStyle(AppTheme.inkMuted)
                                Text("任务拆解与每日奖牌设计经 HTTPS 发送至 DailyQuest Worker，再转发至 DeepSeek。领取任务时生成奖牌；在设置中改任务会重新生成奖牌。历史上的今天优先使用 Wikimedia 公开接口。")
                                    .font(AppTheme.caption(12))
                                    .foregroundStyle(AppTheme.inkMuted)
                                    .lineSpacing(3)
                            }
