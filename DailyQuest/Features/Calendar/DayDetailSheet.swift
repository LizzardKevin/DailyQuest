...
struct DayDetailSheet: View {
    ...
    private func planContent(_ plan: DailyPlan) -> some View {
        ...
        case .base, .holographic:
            let design = MedalDesignService.design(for: plan)
            VStack(spacing: 12) {
                HStack {
                    Spacer()
                    MedalView(status: status, design: design, size: 100)
                    Spacer()
                }
                if let design {
                    Text(design.title)
        ...
        case .inProgress:
            ...
            GlassCard {
                VStack(alignment: .leading, spacing: 10) {
                    if let design {
                        HStack(spacing: 12) {
                            MedalView(status: .base, design: design, size: 44, animateHolographic: false)
                                .opacity(0.7)
                            Text(design.title)
                ...
                GeometryReader { geo in
                    ZStack(alignment: .leading) {
                        Capsule()
                            ...
                            .overlay {
                                Capsule().strokeBorder(AppTheme.glassEdge.opacity(0.65), lineWidth: 0.8)
                            }
                        Capsule()
